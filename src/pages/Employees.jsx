...
function Employees() {
  ...
  const confirmDelete = () => {
    if (employeeIdToDelete) {
      fetch(`http://localhost:8000/api/delete/${employeeIdToDelete}`, {
        method: 'delete'
      })
        .then(data => {
  ...
  const affichEmployee = (id) => {
    window.location.href = `/Page1?id=${id}&enq=${employees && employees.filter((c) => c.id === id).map((c) => c.idenq)[0]}&num=${employees && employees.filter((c) => c.id === id).map((c) => c.num)[0]}`;
  };
...
const handleSearch = () => {
    fetch(`http://127.0.0.1:8000/api/search?term=${searchTerm}`)
      .then(response => response.json())
...
<div className="flex space-x-1">
                ...
                />
                <button onClick={handleSearch} style={{ backgroundColor: currentColor, }} className="px-4 text-white  rounded-full ">
                    <svg
        ...
        <table class="min-w-max w-full table-auto">
            ...
            {searchTerm === '' ? (         
                ...
                <td class="py-3 px-6 text-center">
                        ...
                        <div class="flex item-center justify-center">
                            ...
                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110" onClick={() => handleDelete(employee.id)}>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex items-center ml-6">
                ...
                searchResults.map((employee, index) => (
                  <tr class="border-b border-gray-200 bg-gray-50 hover:bg-gray-100" key={index} >
