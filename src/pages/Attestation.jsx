...
const Attestation = () => {
 ...
 const idsortie = urlParams.get('id');
...
useEffect(() => {
  fetch('http://localhost:8000/api/affSortie')
    .then((response) => response.json())
    .then((data) => {
      setSortie(data);
    })
...
{Sortie &&
                  Sortie
                    .filter((c) => c.numAS === parseInt(idsortie))
                    .map((Sortie) => (
                      <>
...
<div class="container" >
        ...
        <p className="mb-2 mt-2 mr-4 text-gray-900 dark:text-white text-center"> <b> مغادرة المؤسسة  : 
  </b>{demande && demande.filter((c) => c.num == parseInt(candidat && candidat.filter((c) => c.id === parseInt(idBenef) ).map((c) => c.num)[0]) ).map((c) => c.prenom)[0]}  
    {' '}  {demande && demande.filter((c) => c.num === parseInt(candidat && candidat.filter((c) => c.id === parseInt(idBenef) ).map((c) => c.num)[0]) ).map((c) => c.nom)[0]}</p>

            <p className='text-right mb-2 mt-8'><b> المغادرة    </b>{Sortie.numAS} </p>
            <p className='text-right mb-10'>  <b>      تقرير عن أسباب مغادرة المستفيد   المؤسسة   :  </b> {Sortie.remarque}</p>           
            <p className='text-right mb-2'> <b> مراجع الوثائق التي تثبت المغادرة    </b>  
            <img src={require(`../../public/images/${Sortie.file}`)} />
 </p>
            <p className='text-right mb-2'> <b>  رقم و تاريخ محضر لجنة البث في المغادرة : </b>{Sortie.numAS}    {Sortie.dateAS} </p>           
            
            

           
             <span class="inline-grid">
          <p className='text-right mt-14 mb-12'>  <b> : توقيع المدير  </b>  </p>
          <p className='text-right bottom-14 mt-4 absolute left-12'><b> :    
توقيع المرشدة
 الإجتماعية 
 </b> </p>
