 ...
 const { setCurrentColor, setCurrentMode, currentMode, activeMenu, currentColor, themeSettings, setThemeSettings } = useStateContext();
  ...
  const r = (event) => {
   ...
   // Replace 'yourId' with the actual value
    ...
    const url = `/Registre2?id=${yourId}&dateR=${dateR}`;
...
const handleSearch = () => {
    fetch(`http://127.0.0.1:8000/api/searchr?term=${searchTerm}`)
  ...
  const confirmDelete = () => {
    if (employeeIdToDelete) {
      fetch(`http://localhost:8000/api/s/${employeeIdToDelete}`, {
        method: 'delete'
      })
        .then(data => {
  ...
  const affichRegistre = (id) => {
    window.location.href = `/AffichageRegistre2?id=${id}`;
  };
  const editEmployee = (id, dateR) => {
    window.location.href = `/AR2?id=${id}&dateR=${dateR}`;
  };
...
>
                            ...
                            <div class="w-4 mr-2 transform hover:text-purple-500 hover:scale-110" onClick={() => handleDelete(Reg.id)}>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
